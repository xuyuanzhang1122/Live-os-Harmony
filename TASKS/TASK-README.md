...
严格按主控调整后的顺序执行：**0 → 1 → 2 → 6 → 3 → 4 → 5 → 7 → 8**（TASK-6 提前，让用户能尽早配置服务器实测；依赖关系见各任务书「前置条件」）。TASK-2/5/6 本可在 TASK-1 后并行，但保持单线执行便于审查。

## 真机测试里程碑（用户操作，实施者无需执行）

| 里程碑 | 完成标志 | 可测内容 |
|---|---|---|
| M1 | TASK-2 + TASK-6 后 | 首次端到端：填服务器地址、绑 API Key、连接测试、视频库网格/缩略图/轮询刷新、文件列表、删除、缓存管理 |
| M2 | TASK-3 后 | 真实播放（MP4 直链 + HLS 两条路径）、时间线拖动、倍速 |
| M3 | TASK-4 后 | 手势手感、续播、进度上报、PiP、触觉 |
| M4 | TASK-5 后 | 直播间增删启停、观看历史 |
| M5 | TASK-7 后 | 备份导出、短 ID 找回、恢复重启轮询 |
| M6 | TASK-8 后 | 启动动画、平板布局总验收，可日常使用 |

装机前提（一次性）：DevEco Studio 自动签名 + MatePad 开发者模式/USB 调试 + 服务器地址可达（同局域网或公网）。
